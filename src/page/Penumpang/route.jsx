...
// Custom Bus Stop Icon menggunakan SVG
const createBusStopIcon = (id, isMain = false, isHovered = false) => {
  const size = isMain ? 44 : isHovered ? 40 : 36;
  const bgColor = isMain ? "#F59E0B" : "#4285F4";
  const borderColor = isMain ? "#D97706" : "#1E40AF";
  
  const svgIcon = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size + 10}" viewBox="0 0 ${size} ${size + 10}">
      <!-- Shadow -->
      <ellipse cx="${size/2}" cy="${size + 5}" rx="${size/4}" ry="4" fill="rgba(0,0,0,0.2)"/>
      
      <!-- Pin Shape -->
      <path d="M${size/2} ${size + 2} 
               C${size/2} ${size + 2} ${size - 4} ${size * 0.6} ${size - 4} ${size * 0.4}
               C${size - 4} ${size * 0.18} ${size * 0.78} 2 ${size/2} 2
               C${size * 0.22} 2 4 ${size * 0.18} 4 ${size * 0.4}
               C4 ${size * 0.6} ${size/2} ${size + 2} ${size/2} ${size + 2}Z" 
            fill="${bgColor}" 
            stroke="${borderColor}" 
            stroke-width="2"/>
      
      <!-- Bus Icon -->
      <g transform="translate(${size/2 - 10}, ${size * 0.25 - 8})">
        <!-- Bus Body -->
        <rect x="2" y="4" width="16" height="12" rx="2" fill="white"/>
        <!-- Windows -->
        <rect x="4" y="6" width="4" height="4" rx="1" fill="${bgColor}"/>
        <rect x="10" y="6" width="4" height="4" rx="1" fill="${bgColor}"/>
        <!-- Wheels -->
        <circle cx="6" cy="16" r="2" fill="${borderColor}"/>
        <circle cx="14" cy="16" r="2" fill="${borderColor}"/>
        <!-- Front -->
        <rect x="16" y="8" width="2" height="4" rx="0.5" fill="${borderColor}"/>
      </g>
      
      <!-- ID Label -->
      <text x="${size/2}" y="${size * 0.75}" 
            font-family="Arial, sans-serif" 
            font-size="${isMain ? 11 : 9}" 
            font-weight="bold" 
            fill="white" 
            text-anchor="middle" 
...
const createBusStopSignIcon = (id, isMain = false, isHovered = false) => {
  ...
  const bgColor = isMain ? "#DC2626" : "#DC2626"; // Merah seperti di gambar
  ...
  const svgIcon = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${totalWidth}" height="${totalHeight}" viewBox="0 0 ${totalWidth} ${totalHeight}">
      ...
      <!-- Lingkaran utama -->
      <circle cx="${totalWidth/2}" cy="${circleSize/2}" r="${circleSize/2 - 2}" fill="${bgColor}" stroke="#B91C1C" stroke-width="2"/>
...
const RealBusRouteMap = () => {
  ...
  // Koordinat GPS titik jemput dengan detail info
  const busStops = [
    {
      id: "T0",
          ...
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 bg-linear-to-br from-gray-50 to-gray-100">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800">DAFTAR TITIK JEMPUT</h2>
              </div>
                ...
                {busStops.map((stop) => (
                  <div
                    ...
                    className={`p-3 rounded-xl transition-all duration-300 cursor-pointer ${
                      hoveredStop?.id === stop.id || selectedStop?.id === stop.id
            ...
            <div className="bg-gray-800 text-white p-4">
              <div className="text-center text-sm">
                ...
                <p className="text-gray-400 text-xs">18 Titik Jemput • Rute Mengikuti Jalan</p>
              </div>
            </div>
          </div>
        </div>
